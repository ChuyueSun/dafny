// Dafny 3.9.1.41027
// Command Line Options: Test/mytest/seq.dfy /proverLog:Test/mytest/seqdfy.smt2 /print:Test/mytest/seqdfy.bpl
...
axiom (forall n: int :: 
  { char#IsChar(n): bool } 
  char#IsChar(n): bool == (0 <= n && n < 65536));
...
function {:inline} Seq#Build<T>(s: Seq, val: T) : Seq
{
...
function {:builtin "seq.update"} Seq_Update_Sub<T>(s: Seq, i: int, sub: Seq) : Seq;

function {:inline} Seq#Update<T>(s: Seq, i: int, val: T) : Seq
{
...
function {:inline} Seq#Contains<T>(s: Seq, x: T) : bool
{
...
implementation {:verboseName "m (correctness)"} Impl$$_module.__default.m() returns ($_reverifyPost: bool)
{
  ...
  var s#0: Seq where $Is(s#0, TSeq(TInt)) && $IsAlloc(s#0, TSeq(TInt), $Heap);
    ...
    $_reverifyPost := false;
    // ----- assignment statement ----- /home/liviasun/dafny/Test/mytest/seq.dfy(4,11)
    assume true;
    assume true;
    s#0 := Lit(Seq#Build(Seq#Build(Seq#Empty(): Seq, $Box(LitInt(1))), $Box(LitInt(2))));
    // ----- assert statement ----- /home/liviasun/dafny/Test/mytest/seq.dfy(5,5)
